...
router.get('/filtros', async (request, response) => {
  // usando await async
  try {
    let sql = `
        select pes.cpf, pes.nome, pes.tel,
...
router.get('/quantidade/:cargo', async (request, response) => {
  // usando await async
  try {
    const sql = `
...
router.post('/', async (request, response) => {
  ...
  try {
    ...
    if (request.body.pesquisador == true) {
      // insere pesquisador
...
router.put('/', async (request, response) => {
  ...
  try {
    const sql = {
      text:
        'INSERT INTO pessoa ("cpf", "nome", "tel") VALUES ($1, $2, $3) ON CONFLICT ("cpf") DO UPDATE SET nome = excluded.nome, tel = excluded.tel',
      values: [request.body.cpf, request.body.nome, request.body.telefone]
    };
    ...
    if (request.body.pesquisador == true) {
      // insere pesquisador
      await conexao.query(
        `INSERT INTO PESQUISADOR ("cpf") VALUES (${request.body.cpf}) ON CONFLICT("cpf") DO NOTHING`
      );
    } else {
    ...
    if (request.body.jornalista == true) {
      await conexao.query(
        `INSERT INTO JORNALISTA ("cpf") VALUES (${request.body.cpf}) ON CONFLICT("cpf") DO NOTHING`
      );
    } else {
    ...
    if (request.body.produtor == true) {
      await conexao.query(
        `INSERT INTO PRODUTOR ("cpf") VALUES (${request.body.cpf}) ON CONFLICT("cpf") DO NOTHING`
      );
    } else {
    ...
    if (request.body.editor == true) {
      await conexao.query(
        `INSERT INTO EDITOR ("cpf") VALUES (${request.body.cpf}) ON CONFLICT("cpf") DO NOTHING`
      );
    } else {
